...
/** Hardened read-only admin operations panel (auth + diagnostics + audit view). */
...
if (isset($_GET['totp_status'])) {
    ...
    if ($code === 'bad_password') {
        ...
        $notices[] = 'TOTP settings saved. Secret regenerated.';
...
if (isset($_GET['pass_status'])) {
    ...
    if ($code === 'ok') {
        ...
        $inlineErrors['pass_new_password'] = 'Use 6-128 characters.';
    } elseif ($code === 'write_failed') {
...
<body>
  ...
  <main class="shell">
    <header class="top">
      ...
      <p class="muted">Read-only operations dashboard with hardened access controls and live shard diagnostics.</p>
    </header>
    ...
    <?php if (!$isAuthed): ?>
      <section class="login-layout">
        <article class="panel login-card">
          <h2>Sign in</h2>
          <p class="muted">Protected entry point for authorized shard operators.</p>
    ...
    <?php else: ?>
      <section class="panel actions top-actions">
        <div class="chip-row">
          ...
          <span class="chip">2FA: <?= !empty($settings['totp_enabled']) ? 'enabled' : 'disabled' ?></span>
        </div>
      ...
      <section class="grid">
        <article class="panel stat-panel">
          ...
          <ul class="kv-list">
            ...
            <li><strong>Bot last seen (ms):</strong> <?= $bot['botLastSeenMs'] !== null ? (int) $bot['botLastSeenMs'] : 'n/a' ?></li>
        ...
        <article class="panel stat-panel">
          ...
          <?php if ($realm === null): ?>
            <p class="muted">No realm data available.</p>
          <?php else: ?>
            <ul class="kv-list">
      ...
      <section class="panel maintenance-panel">
        ...
        <?php if ($dbStats === null): ?>
          <p class="muted">DB stats unavailable.</p>
        <?php else: ?>
          <ul class="kv-list">
          ...
          <p class="muted">Backup is generated as a snapshot (`VACUUM INTO` when available) and streamed as `.db`.</p>
        <?php endif; ?>
      </section>

      <section class="panel audit-panel">
        <h2>Admin audit trail</h2>
        <p class="muted">Latest admin-kind events from `realm_events`.</p>
        <?php if (count($events) === 0): ?>
          <p class="muted">No admin events yet.</p>
        <?php else: ?>
      ...
      <section class="panel">
        ...
        <ul class="kv-list">
          <li>Use IRC `ADMIN RESTART` for zero-touch bot recycle when watchdog/supervisor is active.</li>
          <li>Use IRC `ADMIN SHUTDOWN` for intentional maintenance stop.</li>
          <li>Keep this panel behind private network and strict IP allowlist in addition to password auth.</li>
          <li>2FA TOTP is <?= !empty($settings['totp_enabled']) ? 'enabled' : 'disabled' ?>.</li>
        </ul>
      </section>

      <section class="panel tool-panel" id="totp-settings">
        <h2>TOTP settings</h2>
        <p class="muted">Enable/disable TOTP and manage secret/issuer (requires current password).</p>
        <?php if ($totpUri !== null && $totpQrUrl !== null): ?>
          <div class="totp-qr-wrap">
            ...
            <div class="totp-qr-meta">
              <p class="muted">Scan with Google Authenticator, Authy, or 1Password.</p>
        ...
        <form method="post" action="/admin/index.php" autocomplete="off" class="admin-form totp-form">
          ...
          <input type="hidden" name="csrf" value="<?= htmlspecialchars(irpg_admin_csrf_token(), ENT_QUOTES, 'UTF-8') ?>" />
          <label>
            <input type="checkbox" name="totp_enabled" value="1" <?= !empty($settings['totp_enabled']) ? 'checked' : '' ?> />
            Enable TOTP second factor
          </label>
          ...
          <?php endif; ?>
          <label>
            <input type="checkbox" name="totp_regenerate" value="1" />
      ...
      <section class="panel tool-panel" id="password-settings">
        ...
        <p class="muted">Applies immediately by writing a new hash into `site.config.php`.</p>
        <form method="post" action="/admin/index.php" autocomplete="off" class="admin-form">
          ...
          <?php endif; ?>
          <label for="new_password">New password (6-128 chars)</label>
